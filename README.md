...
CA (Conway's Life) simulation performance with Python+PyTorch [(CARLE)](https://github.com/rivesunder/carle) on a 4 24-core AMD Threadripper 3960x CPU.
</div>

In this benchmark I used PyTorch's built in convolutions in Python and Fourier transform based convolutions with `FFTW.jl` in Julia. Although `FFTW.jl` has the option to control the number of threads used with `FFTW.set_num_threads`, utilization was never much more than 4 threads and performance actually decreased with more threads in most circumstances. 
